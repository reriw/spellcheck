...
for i, misspelled_word in enumerate(test_word):
    ...
    for word in dictionary:
        l.append(Word(word, Levenshtein.ratio(misspelled_word, word)))
    ...
    if correct_word[i] in predicts:
        corrects = corrects + 1
    # recall = recall + predicts.count(correct_word[i]) / len(predicts)
    # print("{}/{} accuracy: {}".format(i, len(test_word), corrects / (i + 1)))
    # print("{}/{} recal: {}".format(i, len(test_word), predicts.count(correct_word[i]) / len(predicts)))
    with open("ratio-output.txt", "a") as f:
